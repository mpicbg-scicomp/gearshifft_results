...
RESULTSA=$HOME/cuda-workspace/gearshifft_results/results/P100-PCIE-16GB/cuda-8.0.61
RESULTSB=$HOME/cuda-workspace/gearshifft_results/results/P100-PCIE-16GB/clfft-2.12.2
RESULTSC=$HOME/cuda-workspace/gearshifft_results/results/P100-PCIE-16GB/cuda-9.0.69-rc
...
module purge
module load boost/1.60.0-gnu5.3-intelmpi5.1 gcc/5.3.0
module unload mpirt
...
if [ $k -eq 1 ]; then
    ...
    mkdir -p ${RESULTSA}
#    export LD_PRELOAD=$CUDA_ROOT/lib64/libcuda.so
...
if [ $k -eq 3 ]; then
    module load clFFT/2.12.2-cuda8.0-gcc5.3
    module switch cuda/8.0.61_t1
    $CURDIR/release_t1/gearshifft_clfft -l
    mkdir -p ${RESULTSB}
